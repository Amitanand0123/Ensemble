...
import { Loader2, Mail, Briefcase, MapPin, Upload, MessageSquare, Edit, Building, Lock } from 'lucide-react';
...
const UserProfile = () => {
    ...
    // Calculate isOwnProfile based on the loaded profile and loggedInUser
    const isOwnProfile = !!loggedInUser && !!profile && loggedInUser._id === profile._id;

    const [isUploading, setIsUploading] = useState(false);
    const fileInputRef = useRef(null);

    // Fix 1: Handle "me" parameter more gracefully
    useEffect(() => {
        // Clear any previous user data when component mounts or userId changes
    ...
    }, [dispatch, routeUserId, loggedInUser?._id, authIsLoading, isAuthenticated, navigate]);
    ...
    const triggerFileInput = () => {
        if (fileInputRef.current) {
            ...
            console.error("[UserProfile] File input ref not found!");
            toast.error("Could not open file selector.");
        }
    };
    ...
    // Fix 2: Improved loading and error handling
    // "Waiting for authentication" state
    if (routeUserId === 'me' && authIsLoading) {
        return <div className="flex justify-center items-center h-screen text-white"><Loader2 className="h-8 w-8 animate-spin" /> Checking authentication...</div>;
    }
    
    // Profile loading state - only show if we don't have a profile yet or the actual profile is loading
    ...
    // Profile not found state - both loading processes finished but no profile data
    if (!profile && !profileIsLoading && !authIsLoading) {
        return (
            <div className="flex flex-col justify-center items-center h-screen">
                <div className="text-gray-400 mb-4">User profile not found or could not be loaded.</div>
                <Button onClick={() => navigate(-1)}>Go Back</Button>
            </div>
        );
    }

    // Profile should be available here
                    ...
                    <CardHeader className="bg-gradient-to-r from-gray-800 via-gray-800/80 to-gray-800 p-6 border-b border-gray-700">
                        <div className="flex flex-col md:flex-row items-center gap-6">
                            ...
                            <div className="relative group">
                                ...
                                {/* Render upload button based on calculated isOwnProfile */}
                                {isOwnProfile && (
                                    <Button
                                        variant="outline" size="icon"
                                        className="absolute bottom-0 right-0 rounded-full bg-gray-700 hover:bg-gray-600 border-gray-600 text-white w-8 h-8 group-hover:opacity-100 opacity-0 transition-opacity"
                            ...
                            <div className="flex flex-col md:flex-row gap-2 mt-4 md:mt-0 md:ml-auto self-center md:self-start">
                                {/* Render buttons based on calculated isOwnProfile */}
                                {isOwnProfile ? (
                            ...
                            {profile?.workspaces && profile.workspaces.length > 0 ? (
                                <ul className="space-y-2">
                                     ...
                                     {profile.workspaces.filter(w => w.workspace).length === 0 && (
                                        <p className="text-gray-500 italic">No accessible workspaces to display.</p>
                                    )}
                                </ul>
                            ) : (
